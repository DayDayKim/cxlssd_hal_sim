...
void boClass::bosetvalues(uint64_t hsize, uint64_t maxoff, uint64_t  b, uint64_t r)
{
    ...
    while (offsetscore.size() < maxoffset)
        offsetscore.push_back(0);
    while (offsetscore.size() > maxoffset)
        offsetscore.pop_back();
}
...
int64_t leapClass::findoffset()
{
    ...
    while (1)
    {
        ...
        if (delta != 0 || wsize > hbuffer.size() || wsize == 0)
            return delta;
