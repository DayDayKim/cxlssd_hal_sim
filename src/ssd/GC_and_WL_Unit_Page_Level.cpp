...
{
    if (((NVM_Transaction_Flash*)(p1))->PPA < ((NVM_Transaction_Flash*)(p2))->PPA)
        return true;
    else
        return false;
}
...
int GC_and_WL_Unit_Page_Level::write_pages()
{
    ...
    if (waiting_writeback_transaction.size() != 0)
    {
        ...
        if (waiting_writeback_transaction.size() > 1)
        {
            ...
            {
                stop_iterate = false;
                bound_btm_PPA = (((NVM_Transaction_Flash*)(*it))->PPA) / align_unit;
                bound_btm_PPA *= align_unit;
                ...
                for (int i = 1; i < align_unit;)
                {
                    ...
                    if (((NVM_Transaction_Flash*)(*(next(it, i))))->PPA == end_PPA_of_arr || abs(dist) >= align_unit)
                    {
                        stop_iterate = true;
                    }
                    if ((abs(dist) < align_unit) && ((((NVM_Transaction_Flash*)(*(next(it, i))))->PPA >= bound_btm_PPA) && (((NVM_Transaction_Flash*)(*(next(it, i))))->PPA < bound_top_PPA)))
                    {
                        ...
                        waiting_writeback_transaction.erase(next(it, i));
                        i--;
                ...
                //std::cout << std::endl;
                if (*it == waiting_writeback_transaction.back())
                    break;
                if (((NVM_Transaction_Flash*)(*(next(it, 1))))->PPA == end_PPA_of_arr)
                {
                    break;
                }
            }
...
int GC_and_WL_Unit_Page_Level::read_pages()
{
    ...
    if (waiting_submit_transaction.size() > 1)
    {
        for (std::list<NVM_Transaction*>::const_iterator it = waiting_submit_transaction.begin(); it != waiting_submit_transaction.end(); it++)
        {
            ...
            if (it == waiting_submit_transaction.end())
                break;
            stop_iterate = false;
            for (int idx = 1; idx < (waiting_submit_transaction.size() - (front_idx));)
            {
                if ((next(it, idx)) == waiting_submit_transaction.end())
                {
                    stop_iterate = true;
                };
